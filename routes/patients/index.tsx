...
export async function handler(req: Request, ctx: FreshContext<AppState>) {
  ...
  try {
    ...
    if (currentUser.role === "superadmin") {
      ...
      allPatients = await patientRepository.getAllPatientsAsProfiles();
    } else {
      // Psicólogos solo ven pacientes con los que han tenido citas
      const userAppointments = await appointmentRepository
        .getAppointmentsByPsychologist(
          currentUser.email,
        );
      const patientNamesWithAppointments = new Set(
        userAppointments.map((apt) => apt.patientName),
      );

      const allPatientsData = await patientRepository
        .getAllPatientsAsProfiles();
      allPatients = allPatientsData.filter((patient) =>
